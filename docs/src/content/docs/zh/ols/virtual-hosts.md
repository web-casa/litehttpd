...
OpenLiteSpeed 内置的 .htaccess 支持有限。要获得完整的 Apache 兼容 .htaccess 处理能力，请使用 LiteHTTPD 模块（`litehttpd_htaccess.so`）。
...
- **autoLoadHtaccess** -- 加载 LiteHTTPD 时保持为 `0`。OLS 原生 `.htaccess` 处理能力有限，且可能与 LiteHTTPD 双重处理相同指令。
