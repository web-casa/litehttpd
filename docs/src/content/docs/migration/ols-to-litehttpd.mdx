...
Stock OLS only handles a small subset of `.htaccess` directives (primarily `RewriteFile` for basic rewrite rules). LiteHTTPD adds 83 directive types including `Header`, `Require`, `FilesMatch`, `AuthType Basic`, `If/ElseIf/Else`, and more.

## What You Gain

| Feature | Stock OLS | With LiteHTTPD |
|---------|-----------|----------------|
| .htaccess directive types | ~6 (RewriteFile only) | **83** |
